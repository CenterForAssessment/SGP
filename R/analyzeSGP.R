...
function(sgp_object,
	...
	if (get.cohort.data.info) {
		cohort_data_infor <- getCohortDataInfo(tmp_sgp_data_for_analysis, par.sgp.config)
			...
			if (toupper(parallel.config[["BACKEND"]]) == "FOREACH") {
				tmp <- foreach(sgp.iter=iter(rev(par.sgp.config.baseline)), .packages="SGP", .combine="mergeSGP", .inorder=FALSE,
					...
					return(studentGrowthPercentiles(
						...
						content.area.progression=tail(sgp.iter[["sgp.content.areas"]], length(sgp.iter[["base.gp"]])),
