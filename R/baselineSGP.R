...
# stop("cok$")
    ...
    if (is.null(SGPstateData[[state]][["Baseline_splineMatrix"]])) {
        if (missing(sgp.baseline.config)) {
            ...
            for (a in seq_along(tmp.sgp.baseline.grade.sequences)) {
                ...
                eval(parse(text=paste("list(", paste("tail(tmp.sgp.baseline.grade.sequences[[", a, "]],", 
                    length(tmp.sgp.baseline.grade.sequences[[a]]):2, ")", collapse=", "), ")")))
            }
    ...
    for (sgp.iter in sgp.config) {
        ...
        sgp.vnames <- c("ID", paste("GRADE", sgp.iter[["sgp.panel.years"]], sep="."), paste("SCALE_SCORE", sgp.iter[["sgp.panel.years"]], sep="."))
