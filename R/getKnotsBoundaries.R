getKnotsBoundaries <- function(
    ...
    ### Utility functions
    .get_content_Areas <- function(sgp.iter, type) {
    ...
    if (sgp.iter.type == "sgp.percentiles") {
        tmp.content_areas <- .get_content_Areas(sgp.iter, 'sgp.content.areas')
    ...
    if (sgp.iter.type == "sgp.percentiles.baseline") {
        tmp.content_areas <- .get_content_Areas(sgp.iter, 'sgp.baseline.content.areas')
        content_area.label <- tail(sgp.iter[["sgp.baseline.content.areas"]], 1)
        if (!is.null(sgp.year.baseline)) tmp.year <- sgp.year.baseline else tmp.year <- tail(sgp.iter[["sgp.panel.years"]], 1)
    }
    if (sgp.iter.type == "sgp.projections") {
        tmp.content_areas <- .get_content_Areas(sgp.iter, 'sgp.projection.content.areas')
    ...
    if (sgp.iter.type == "sgp.projections.baseline") {
        tmp.content_areas <- .get_content_Areas(sgp.iter, 'sgp.projection.baseline.content.areas')
