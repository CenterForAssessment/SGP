	...
	get.par.sgp.config <- function(sgp.config) {
		...
		for (a in seq_along(sgp.config)) { # now seq_along names so that sgp.config lists can have same names for some elements
			...
			for (b in seq_along(sgp.config[[a]][['sgp.grade.sequences']])) {
				...
				if (any(duplicated(paste(par.sgp.config[[cnt]][['sgp.panel.years']], par.sgp.config[[cnt]][['sgp.grade.sequences']], sep=".")))) {  
				...
				par.sgp.config[[cnt]][['sgp.grade.sequences']] <- as.character(par.sgp.config[[cnt]][['sgp.grade.sequences']])

				if (is.numeric(type.convert(par.sgp.config[[cnt]][['sgp.grade.sequences']]))) {
					grade.span <- seq(min(type.convert(par.sgp.config[[cnt]][['sgp.grade.sequences']])),
						max(type.convert(par.sgp.config[[cnt]][['sgp.grade.sequences']])))
					index <- match(type.convert(par.sgp.config[[cnt]][['sgp.grade.sequences']]), grade.span)
				...
				if (sgp.percentiles.baseline | sgp.projections.baseline | sgp.projections.lagged.baseline) {
					...
					if (paste(strsplit(names(sgp.config)[a], "\\.")[[1]][1], ".BASELINE", sep="") %in% names(tmp_sgp_object[["Coefficient_Matrices"]])) {
						...
						mtx.index <- which(sapply(mtx.names, function(x) strsplit(x, "_")[[1]][2]) == tail(par.sgp.config[[cnt]][['sgp.grade.sequences']], 1))
						if (length(mtx.index) > 0) { # Cases where content area has some BASELINE coef matrices, but not with the present grade prog
							...
							if (length(par.sgp.config[[cnt]][['sgp.grade.sequences']])-1 < tmp.max.order) {
								tmp.max.order <- length(par.sgp.config[[cnt]][['sgp.grade.sequences']])-1
							}
							tmp.orders <- getsplineMatrices(
								my.matrices=tmp.matrices[mtx.index], 
								my.matrix.content_area.progression=tail(par.sgp.config[[cnt]][['sgp.content.areas']], tmp.max.order+1), 
								my.matrix.grade.progression=tail(par.sgp.config[[cnt]][['sgp.grade.sequences']], tmp.max.order+1), 
							...
							if (par.sgp.config[[cnt]][['sgp.exact.grade.progression']] && !tmp.max.order %in% tmp.orders) {
					...
					if (!tmp.matrices.tf) {
						...
						par.sgp.config[[cnt]][['base.gp']] <- as.character(tail(par.sgp.config[[cnt]][['sgp.grade.sequences']], tmp.max.order+1))
