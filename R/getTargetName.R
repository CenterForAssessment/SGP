...
function(state,
	...
	if (!is.null(projection_group.iter) && projection_group.iter %in% names(SGPstateData[[state]][['SGP_Configuration']][['grade.projection.sequence']])) {
		target.years <- SGPstateData[[state]][['SGP_Configuration']][['max.forward.projection.sequence']][[projection_group.iter]]
	}
