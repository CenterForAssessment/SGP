	...
	if (calculate.sgps) {
		...
		if (simex.tf) {
			...
			if(!is.null(quantile.data.simex[['MATRICES']])) {
				tmp_sgp_1 <- list(Coefficient_Matrices = Coefficient_Matrices[[paste(tmp.path.coefficient.matrices, '.SIMEX', sep="")]][[paste('grade_', tail(tmp.gp,1), sep="")]])
				tmp_sgp_2 <- list(Coefficient_Matrices = quantile.data.simex[['MATRICES']][[paste('grade_', tail(tmp.gp,1), sep="")]])
				...
				Coefficient_Matrices[[paste(tmp.path.coefficient.matrices, '.SIMEX', sep="")]][[paste('grade_', tail(tmp.gp,1), sep="")]] <- 
					tmp_sgp_combined[["Coefficient_Matrices"]][[paste('grade_', tail(tmp.gp,1), sep="")]]
			}
		}
