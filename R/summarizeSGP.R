`summarizeSGP` <- function(
	...
	if (summarizeSGP.baseline) {
		my.sgp <- intersect(names(slot.data), c("SGP", "SGP_BASELINE", "SGP_SIMEX", "SGP_SIMEX_BASELINE")) 
	} else {
		my.sgp <- intersect(names(slot.data), c("SGP", "SGP_SIMEX"))
	}
