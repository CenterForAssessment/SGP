...
+                         grep("Achievement_Levels", names(SGP::SGPstateData[[state]][["Assessment_Program_Information"]][["Assessment_Transition"]]))]
			...
			tmp.sgp.summaries <- list(
				...
				PERCENT_AT_ABOVE_PROFICIENT=paste("percent_in_category_1(ACHIEVEMENT_LEVEL, ",
				...
				if ("ACHIEVEMENT_LEVEL_PRIOR" %in% sgp.data.names) {
					tmp.sgp.summaries <- c(
						tmp.sgp.summaries,
						PERCENT_AT_ABOVE_PROFICIENT_PRIOR=paste("percent_in_category_2(ACHIEVEMENT_LEVEL_PRIOR, ",
							get.expression(proficient.achievement.levels), ", ", get.expression(all.achievement.levels), ")",sep=""),
				...
				if (!is.null(my.sgp.target)) {
					tmp.sgp.summaries <- c(
						...
						PERCENT_CATCHING_UP_KEEPING_UP="percent_in_category_3(CATCH_UP_KEEP_UP_STATUS, c('Catch Up: Yes', 'Keep Up: Yes'), c('Catch Up: Yes', 'Catch Up: No', 'Keep Up: Yes', 'Keep Up: No'))"
					)
				...
				if (!is.null(my.sgp.target.baseline)) {
					tmp.sgp.summaries <- c(
						...
						PERCENT_CATCHING_UP_KEEPING_UP_BASELINE="percent_in_category_4(CATCH_UP_KEEP_UP_STATUS_BASELINE, c('Catch Up: Yes', 'Keep Up: Yes'), c('Catch Up: Yes', 'Catch Up: No', 'Keep Up: Yes', 'Keep Up: No'))"
					)
					names(tmp.sgp.summaries)[sapply(c("M1", "M2"), function(x) which(names(tmp.sgp.summaries)==x))] <-
				...
				if (!is.null(my.sgp.target.musu)) {
					tmp.sgp.summaries <- c(
						...
						PERCENT_MOVING_UP_STAYING_UP="percent_in_category_5(MOVE_UP_STAY_UP_STATUS, c('Move Up: Yes', 'Stay Up: Yes'), c('Move Up: Yes', 'Move Up: No', 'Stay Up: Yes', 'Stay Up: No'))"
					)
					names(tmp.sgp.summaries)[sapply(c("M1", "M2"), function(x) which(names(tmp.sgp.summaries)==x))] <-
				...
				if (!is.null(my.sgp.target.musu.baseline)) {
					tmp.sgp.summaries <- c(
						...
						PERCENT_MOVING_UP_STAYING_UP_BASELINE="percent_in_category_6(MOVE_UP_STAY_UP_STATUS_BASELINE, c('Move Up: Yes', 'Stay Up: Yes'), c('Move Up: Yes', 'Move Up: No', 'Stay Up: Yes', 'Stay Up: No'))"
					)
					names(tmp.sgp.summaries)[sapply(c("M1", "M2"), function(x) which(names(tmp.sgp.summaries)==x))] <-
