...
+                         grep("Achievement_Levels", names(SGP::SGPstateData[[state]][["Assessment_Program_Information"]][["Assessment_Transition"]]))]
			...
			tmp.sgp.summaries <- list(
				...
				PERCENT_AT_ABOVE_PROFICIENT=paste("percent_in_category_NEW_2(data.table(X=ACHIEVEMENT_LEVEL), ",
				...
				if ("ACHIEVEMENT_LEVEL_PRIOR" %in% sgp.data.names) {
					tmp.sgp.summaries <- c(
						tmp.sgp.summaries,
						PERCENT_AT_ABOVE_PROFICIENT_PRIOR=paste("percent_in_category_NEW_2(data.table(X=ACHIEVEMENT_LEVEL_PRIOR), ",
							get.expression(proficient.achievement.levels), ", ", get.expression(all.achievement.levels), ")",sep=""),
				...
				if (!is.null(my.sgp.target)) {
					tmp.sgp.summaries <- c(
						...
						PERCENT_CATCHING_UP_KEEPING_UP="percent_in_category_NEW_2(data.table(X=CATCH_UP_KEEP_UP_STATUS), c('Catch Up: Yes', 'Keep Up: Yes'), c('Catch Up: Yes', 'Catch Up: No', 'Keep Up: Yes', 'Keep Up: No'))"
					)
				...
				if (!is.null(my.sgp.target.baseline)) {
					tmp.sgp.summaries <- c(
						...
						PERCENT_CATCHING_UP_KEEPING_UP_BASELINE="percent_in_category_NEW_2(data.table(CATCH_UP_KEEP_UP_STATUS_BASELINE), c('Catch Up: Yes', 'Keep Up: Yes'), c('Catch Up: Yes', 'Catch Up: No', 'Keep Up: Yes', 'Keep Up: No'))"
					)
					names(tmp.sgp.summaries)[sapply(c("M1", "M2"), function(x) which(names(tmp.sgp.summaries)==x))] <-
