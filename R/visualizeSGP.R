...
####################################################################################################################
	...
	if ("growthAchievementPlot" %in% plot.types) {
        ...
        call_function <- define_compute(parallel.config, "GA_PLOTS")
        results <- lapply(
            gaPlot.list,
            \(gaPlot.iter) {
                list(
            ...
            call_function(growthAchievementPlot)
...
if ("studentGrowthPlot" %in% plot.types) {
	...
	get.next.content_area <- function(grade, content_area, increment) {
		if (!is.null(tmp.domain <- SGP::SGPstateData[[state]][["Student_Report_Information"]][["Content_Areas_Domains"]][[content_area]]) & increment != 0 & !is.na(grade)) {
...
######################################################################
	...
	#### Some checks
		...
		if (!is.null(sgPlot.sgp.targets)) {
			if (!is.null(SGP::SGPstateData[[state]][['SGP_Configuration']][['sgPlot.sgp.targets.timeframe']])) {
				...
				if (!is.null(SGP::SGPstateData[[state]][['SGP_Configuration']][['max.sgp.target.years.forward']])) {
					sgPlot.sgp.targets.timeframe <- max(SGP::SGPstateData[[state]][['SGP_Configuration']][['max.sgp.target.years.forward']])
				}
			}
		}
		...
		## Calculate School size to order from largest to smallest and set key on WIDE data
			...
			if (!is.null(sgPlot.schools) & !is.null(sgPlot.districts)) {
				...
				for (i in seq_along(sgPlot.districts)) {
					...
					if (any(tmp.schools %in% sgPlot.schools)) {
						tmp.list[[i]] <- tmp.schools[tmp.schools %in% sgPlot.schools]
					} else {
...
###############################################
	...
	if (sgPlot.reports.by.instructor) {
		...
		tmp.district.and.schools.instructors <- unique(data.table(student.teacher.lookup, key=long.key)[,district.and.school.variable.names, with=FALSE], by=long.key)
	...
	#### Reshape data (NOT NECESSARY IF WIDE data is provided)
		...
		sgPlot.data[, GRADE := get(paste("GRADE", tmp.last.year, sep="."))]

		variables.to.keep <- c("ID", "CONTENT_AREA", "GRADE", paste("CONTENT_AREA_LABELS", tmp.years.subset, sep="."),
    ...
    #### Merge in 1 year projections (if requested & available) and transform using
    #### piecewiseTransform (if required) (NOT NECESSARY IF WIDE data is provided)
    #### Merge in scale scores associated with SGP_TARGETs (if requested & available) and
    #### transform using piecewiseTransform (if required) (NOT NECESSARY IF WIDE data is provided)
		...
		if (sgPlot.fan | !is.null(sgPlot.sgp.targets)) {
			...
			if (sgPlot.fan & any(tmp.proj.names %in% names(sgp_object@SGP[["SGProjections"]]))) {
				...
				for (i in tmp.proj.names) {
					tmp.proj.names <- grep("PROJ_YEAR_1", names(sgp_object@SGP[["SGProjections"]][[i]]), value = TRUE)
					tmp.list[[i]] <- data.table(CONTENT_AREA = unlist(strsplit(i, "[.]"))[1],
						sgp_object@SGP[["SGProjections"]][[i]][, c("ID", "GRADE", tmp.proj.names), with = FALSE])
				}
				sgPlot.data <- rbindlist(tmp.list, fill = TRUE)[sgPlot.data, on = c("ID", "CONTENT_AREA", "GRADE")]
			} ### END if (sgPlot.fan)
			...
			if (any(c("sgp.projections", "sgp.projections.baseline") %in% sgPlot.sgp.targets) & any(tmp.proj.cut_score.names %in% names(sgp_object@SGP[["SGProjections"]]))) {
				...
				for (i in tmp.proj.cut_score.names) {
					tmp.proj.names <- grep(paste(sgPlot.sgp.targets.timeframe, "YEAR_PROJ", sep = "_"),
					                       names(sgp_object@SGP[["SGProjections"]][[i]]), value = TRUE)
					tmp.list[[i]] <- data.table(CONTENT_AREA = unlist(strsplit(i, "[.]"))[1],
						sgp_object@SGP[["SGProjections"]][[i]][, c("ID", "GRADE", tmp.proj.names), with = FALSE])
				}
				sgPlot.data <- rbindlist(tmp.list, fill = TRUE)[sgPlot.data, on = c("ID", "CONTENT_AREA", "GRADE")]
			...
			if (any(c("sgp.projections.lagged", "sgp.projections.lagged.baseline") %in% sgPlot.sgp.targets) &
				...
				setkeyv(sgPlot.data, c("ID", "CONTENT_AREA", "GRADE"))
				tmp.list <- list()
	...
	#### Merge in INSTRUCTOR_NAME if requested
		if (sgPlot.reports.by.instructor) {
			...
			unique.teacher.lookup <- unique(student.teacher.lookup, by=key(student.teacher.lookup))[,c("ID", paste(c("INSTRUCTOR_NUMBER", "INSTRUCTOR_NAME"), tmp.last.year, sep=".")), with=FALSE]
