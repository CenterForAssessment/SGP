...
The data set `sgpData` is an anonymized, panel data set comprising 5 years of annual, vertically scaled, assessment data in WIDE format. This exemplar data set
...
head(sgpData)
```
The Wide data format illustrated by `sgpData` and utilized by the SGP package can accommodate any number of occurrences but must follow a specific column order.
...
The data set `sgpData_LONG` is an anonymized, panel data set comprising 5 years of annual, vertically scaled, assessment data in LONG format for two content areas
...
We recommend LONG formatted data for use with operational analyses. Managing data in long format is more simple than data in the wide format. For example,
...
The data set `sgpData_INSTRUCTOR_NUMBER` is an anonymized, student-instructor lookup table that provides instructor information associated with each students test record.
